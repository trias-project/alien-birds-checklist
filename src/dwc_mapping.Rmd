...
```{r read_source_data}
input_data <- retrieve_spreadsheet %>% gs_read("checklist", col_types = col_types) # Also trims values
```
...
distribution %<>% mutate(dwc_locationID = case_when(
  ...
  dwc_locality == "Brussels-Capital region" ~ "ISO_3166-2:BE-BRU"))
```
...
pathways %<>% 
  separate(col = introduction_pathway,
           into = c("introduction_pathway_1", "introduction_pathway_2"),
           sep = "\\s\\|\\s",
           remove = FALSE)
```
...
pathways %<>% 
  gather(key = "key",
         value = "value",
         introduction_pathway_1, introduction_pathway_2,
         na.rm = TRUE)
```
...
degree_of_establishment %<>% 
  mutate(description = 
           recode(degree_of_establishment,
              "captive" = "captive (B1)",
              "casual" = "casual (C1)",
              "colonizing" = "colonizing (D1)",
              "established" = "established (C3)",
              "invasive" = "invasive (D2)",
              "released" = "released (B3)",
              "reproducing" = "reproducing (C2)"
))
```

Add prefix `Blackburn et al. 2011`:

```{r}
degree_of_establishment %<>% mutate(description = paste("Blackburn et al. 2011", description, sep = ":"))
```
